...
const CartItem = ({ course, onRemove }) => {
  ...
  return (
    <motion.div
      ...
      className='bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-all duration-300 mb-3 overflow-hidden'
    ...
    >
      <div className='flex p-3 relative'>
        ...
        <div className='flex-1 min-w-0'>
          <div className='flex justify-between items-start'>
            <h2 className='text-sm sm:text-base font-semibold text-gray-800 line-clamp-2 pr-6'>
              {course.title}
            </h2>
            <motion.button
              onClick={() => onRemove(course._id)}
              className='absolute top-2 right-2 text-gray-400 hover:text-red-600 transition-colors duration-200 p-1'
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
            >
              <X size={16} />
          ...
          <p className='text-xs text-gray-600 mt-0.5 line-clamp-1'>
            ...
            <span className='text-gray-500 ml-0.5'>({course.reviews})</span>
          </p>
      ...
      {(!isMobile || expanded) && (
        <div className='px-3 pb-3 pt-0 text-xs border-t border-gray-100 mt-1'>
          <div className='grid grid-cols-2 sm:flex sm:flex-wrap sm:items-center gap-y-1 gap-x-3 mt-2 text-gray-600'>
            <div className='flex items-center'>
          ...
          {course.tags && course.tags.length > 0 && (
            <div className='flex flex-wrap gap-1 mt-2'>
              {course.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className='inline-flex bg-blue-50 text-blue-700 text-xs px-1.5 py-0.5 rounded-full'
                >
                  {tag}
                </span>
              ))}
...
const CartSummary = ({ cart, onCheckout }) => {
  ...
  const originalTotal = cart.reduce(
    (total, course) => total + (course.originalPrice || course.price),
    0
  );
  ...
  return (
    ...
    >
      ...
      <div className='space-y-3'>
        ...
        <div className='flex justify-between text-base font-bold text-gray-800 border-t border-dashed border-gray-200 pt-3 mt-2'>
          <span>Total:</span>
          <motion.span
            key={totalPrice}
            initial={{ scale: 1.1 }}
      ...
      <div className='mt-3 bg-blue-50 rounded-md p-2.5 text-xs text-blue-800 grid grid-cols-2 gap-2'>
        <div className='flex items-center'>
      ...
      <div className='mt-3 flex items-center justify-center text-xs text-emerald-700 bg-emerald-50 rounded-md p-2'>
  ...
  >
    <div className='p-6 pb-8 text-center'>
      <motion.div
        ...
        className='w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-4 flex items-center justify-center bg-blue-50 rounded-full'
      >
      ...
      >
        <Button
          ...
          className='bg-white hover:bg-black hover:text-white text-black border-2 px-4 py-2 rounded-md shadow-sm'
        >
          Explore Courses
        </Button>
      </motion.div>
...
const Cart = () => {
  ...
  return (
    <div className='min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 pt-16 pb-12 mt-24'>
      <div className='px-3 sm:px-6 max-w-6xl mx-auto'>
        <div className='flex justify-center mb-5'>
          <motion.div
            ...
            transition={{ duration: 0.3 }}
            className='bg-white px-4 py-2 rounded-full inline-flex items-center shadow-sm'
          >
            ...
            <div className='lg:col-span-2'>
              ...
              {isMobile && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className='fixed bottom-0 left-0 right-0 p-3 bg-white border-t border-gray-200 z-10'
                >
